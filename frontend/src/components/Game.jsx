...
const Game = () => {
  ...
  const handleMoneyUpdate = async () => {
    try {
      ...
      if (squad.money < priceNumber) {
        ...
        showSuccessAlert(
          "Éxito",
          "El dinero del equipo se ha actualizado correctamente"
        );
      }
    } catch (error) {
      showErrorAlert(
        "Error",
        "Se produjo un error al actualizar el dinero del equipo"
      );
    }
  };
  ...
  return (
    <div className="container bg-white p-5 rounded-md m-10 mx-auto">
      ...
      <div className="mb-4">
        <label htmlFor="squad" className="block mb-1">
          Equipo:
        </label>
        <select
          id="squad"
          ...
          {players.map((player) => (
            <div key={player.id} className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <label htmlFor={`player-${player.id}`} className="text-lg">
                {player.username}
              </label>
            </div>
          ))}
